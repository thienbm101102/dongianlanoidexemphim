...
// GỌI HÀM QUÉT KHO ĐỒ VÀ MỞ KHÓA VẬT PHẨM
	...
	// ==========================================
    // MINIGAME XÉ PACK THẺ BÀI (HARUNO TCG)
    // ==========================================
    tcgData: {
        cost: 1000,
        isOpening: false,
        flippedCount: 0,
        currentPull: [],
        inventory: {}, // Lưu thẻ { 'C1': 5, 'L1': 1 }
        
        // Tỷ lệ xuất hiện: C(50%), UC(30%), R(15%), E(4%), L(1%)
        pool: [
            // Huyền Thoại Hologram (1%)
            { id: 'L1', name: 'Rồng Thượng Cổ', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069172.png', rarity: 'L', label: 'LEGENDARY' },
            { id: 'L2', name: 'Cyber Haruno', img: 'https://cdn-icons-png.flaticon.com/512/2855/2855523.png', rarity: 'L', label: 'LEGENDARY' },
            // Cực Hiếm (4%)
            { id: 'E1', name: 'Pháp Sư Vô Cực', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069186.png', rarity: 'E', label: 'EPIC' },
            { id: 'E2', name: 'Sát Thủ Bóng Đêm', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069162.png', rarity: 'E', label: 'EPIC' },
            { id: 'E3', name: 'Hiệp Sĩ Thánh', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069165.png', rarity: 'E', label: 'EPIC' },
            // Hiếm (15%)
            { id: 'R1', name: 'Sói Lửa', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069151.png', rarity: 'R', label: 'RARE' },
            { id: 'R2', name: 'Tinh Linh Nước', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069155.png', rarity: 'R', label: 'RARE' },
            { id: 'R3', name: 'Thợ Săn Tiền Thưởng', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069176.png', rarity: 'R', label: 'RARE' },
            // Khá (30%)
            { id: 'UC1', name: 'Giáp Sắt', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069179.png', rarity: 'UC', label: 'UNCOMMON' },
            { id: 'UC2', name: 'Thuốc Hồi Máu', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069147.png', rarity: 'UC', label: 'UNCOMMON' },
            { id: 'UC3', name: 'Nhẫn Ma Thuật', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069149.png', rarity: 'UC', label: 'UNCOMMON' },
            // Thường (50%)
            { id: 'C1', name: 'Kiếm Gỗ', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069142.png', rarity: 'C', label: 'COMMON' },
            { id: 'C2', name: 'Bánh Mì', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069143.png', rarity: 'C', label: 'COMMON' },
            { id: 'C3', name: 'Đồng Xu Rỉ Sét', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069144.png', rarity: 'C', label: 'COMMON' },
            { id: 'C4', name: 'Bản Đồ Cũ', img: 'https://cdn-icons-png.flaticon.com/512/3069/3069146.png', rarity: 'C', label: 'COMMON' }
        ]
    },

    openTCG() {
        const email = localStorage.getItem('haruno_email');
        if (!email) { this.openAuthModal(); return this.showToast("Cần đăng nhập để Mua Pack!", "warning"); }
        
        const safeUser = this.getSafeKey(email);

        // Lắng nghe Ví HCoins và Túi Đồ TCG
        if(db) {
            db.ref(`users/${safeUser}/coins`).on('value', snap => {
                const el = document.getElementById('tcg-user-coins');
                if(el) el.innerText = (snap.val() || 0).toLocaleString();
            });

            db.ref(`users/${safeUser}/tcg_inventory`).on('value', snap => {
                this.tcgData.inventory = snap.val() || {};
            });
        }

        document.getElementById('tcg-modal').style.display = 'flex';
        this.switchTCGTab('shop');
    },

    closeTCG() {
        if(this.tcgData.isOpening && this.tcgData.flippedCount < 5) {
            return this.showToast("Hãy lật hết bài trước khi thoát!", "warning");
        }
        ...
        if(email && db) {
            const safeUser = this.getSafeKey(email);
            db.ref(`users/${safeUser}/coins`).off('value');
            db.ref(`users/${safeUser}/tcg_inventory`).off('value');
        }
    },

    switchTCGTab(tab) {
        document.getElementById('tcg-shop-screen').style.display = 'none';
        document.getElementById('tcg-open-screen').style.display = 'none';
        document.getElementById('tcg-collection-screen').style.display = 'none';

        if(tab === 'shop') {
            document.getElementById('tcg-shop-screen').style.display = 'flex';
        } else if (tab === 'open') {
            document.getElementById('tcg-open-screen').style.display = 'flex';
        } else if (tab === 'collection') {
            document.getElementById('tcg-collection-screen').style.display = 'block';
            this.renderTCGCollection();
        }
    },

    buyTCGPack() {
        if(this.tcgData.isOpening) return;
        const email = localStorage.getItem('haruno_email');
        if(!email) return;
        const safeUser = this.getSafeKey(email);

        const packVisual = document.getElementById('tcg-pack-visual');
        packVisual.classList.add('tearing');

        if(this.sounds) this.playSound('click'); // Có thể thay bằng tiếng xé giấy

        // Trừ tiền bằng Transaction Firebase
        db.ref(`users/${safeUser}/coins`).transaction((currentCoins) => {
            if ((currentCoins || 0) >= this.tcgData.cost) {
                return currentCoins - this.tcgData.cost;
            } else {
                return; // Hủy transaction nếu không đủ tiền
            }
        }, (error, committed, snapshot) => {
            if (error || !committed) {
                packVisual.classList.remove('tearing');
                return this.showToast(`Bạn không đủ ${this.tcgData.cost.toLocaleString()} HC để mua Pack!`, "error");
            }

            // Giao dịch thành công -> Tạo 5 thẻ
            this.tcgData.isOpening = true;
            this.tcgData.flippedCount = 0;
            this.tcgData.currentPull = [];

            // Thuật toán Gacha TCG
            for(let i=0; i<5; i++) {
                const rand = Math.random() * 100;
                let targetRarity = 'C';
                if (rand < 1) targetRarity = 'L';       // 1%
                else if (rand < 5) targetRarity = 'E';  // 4%
                else if (rand < 20) targetRarity = 'R'; // 15%
                else if (rand < 50) targetRarity = 'UC';// 30%

                const possibleCards = this.tcgData.pool.filter(c => c.rarity === targetRarity);
                const winCard = possibleCards[Math.floor(Math.random() * possibleCards.length)];
                this.tcgData.currentPull.push(winCard);
                
                // Cộng vào biến inventory tạm
                if(!this.tcgData.inventory[winCard.id]) this.tcgData.inventory[winCard.id] = 0;
                this.tcgData.inventory[winCard.id]++;
            }

            // Lưu inventory lên Firebase
            db.ref(`users/${safeUser}/tcg_inventory`).set(this.tcgData.inventory);

            // Đợi vỏ pack rung 1 giây rồi nổ thành 5 thẻ
            setTimeout(() => {
                packVisual.classList.remove('tearing');
                this.switchTCGTab('open');
                this.renderCardsFaceDown();
            }, 1000);
        });
    },
    ...
    renderCardsFaceDown() {
        ...
        document.getElementById('btn-tcg-done').style.display = 'none';
        container.innerHTML = '';

        this.tcgData.currentPull.forEach((card, index) => {
            // Style delay giúp thẻ hiện ra từ từ như chia bài
            const delay = index * 0.1; 
            const cardHtml = `
                <div class="tcg-card-wrapper" id="tcg-card-${index}" style="animation-delay: ${delay}s" onclick="app.flipTCGCard(${index}, '${card.id}')">
                    <div class="tcg-card-inner">
                        <div class="tcg-card-back"></div>
                        <div class="tcg-card-front rarity-${card.rarity}">
                            <div class="card-img-box">
                                <img src="${card.img}">
                            </div>
                            <div class="card-info">
                                <h4>${card.name}</h4>
                                <span class="card-rarity">${card.label}</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    flipTCGCard(index, cardId) {
        const cardEl = document.getElementById(`tcg-card-${index}`);
        if(cardEl.classList.contains('flipped')) return; 
        
        cardEl.classList.add('flipped');
        this.tcgData.flippedCount++;

        const cardObj = this.tcgData.pool.find(c => c.id === cardId);

        // Hiệu ứng âm thanh & hình ảnh tùy theo độ hiếm
        if (cardObj.rarity === 'L' || cardObj.rarity === 'E') {
            if(this.sounds) this.playSound('win'); 
            if(typeof this.fireJackpotEffect === 'function' && cardObj.rarity === 'L') this.fireJackpotEffect();
            this.showToast(`🔥 KINH ĐIỂN! Lật được ${cardObj.name} (${cardObj.label})!`, "success");
        } else if (cardObj.rarity === 'R') {
            if(this.sounds) this.playSound('coin'); 
        } else {
            if(this.sounds) this.playSound('click'); 
        }

        // Hiện nút XONG nếu đã lật đủ 5 lá
        if (this.tcgData.flippedCount === 5) {
            this.tcgData.isOpening = false;
            setTimeout(() => {
                document.getElementById('btn-tcg-done').style.display = 'block';
            }, 500);
        }
    },
    ...
    renderTCGCollection() {
        ...
        // Sắp xếp thẻ từ Xịn nhất xuống cùi nhất
        const sortedPool = [...this.tcgData.pool].sort((a, b) => {
            const weight = { 'L': 5, 'E': 4, 'R': 3, 'UC': 2, 'C': 1 };
            return weight[b.rarity] - weight[a.rarity];
        });

        sortedPool.forEach(card => {
            const qty = this.tcgData.inventory[card.id] || 0;
            if (qty > 0) {
                hasCards = true;
                grid.innerHTML += `
                    <div class="col-card-wrap">
                        <div class="col-qty">${qty}</div>
                        <div class="tcg-card-wrapper flipped" style="animation: none; transform: scale(1);">
                            <div class="tcg-card-inner">
                                <div class="tcg-card-front rarity-${card.rarity}">
                                    <div class="card-img-box"><img src="${card.img}"></div>
                                    <div class="card-info">
                                        <h4>${card.name}</h4>
                                        <span class="card-rarity">${card.label}</span>
                                    </div>
        ...
        if (!hasCards) {
            grid.innerHTML = `<div style="grid-column: 1/-1; text-align: center; color: #888; padding: 40px;"><i class="fas fa-ghost" style="font-size: 40px; margin-bottom: 10px;"></i><p>Bạn chưa có thẻ nào. Mua Pack để bắt đầu sư tập nhé!</p></div>`;
        }
    },
