...
// ==========================================
	...
	// ==========================================
    // MINIGAME ĐẬP HỘP GACHA (MYSTERY BOX)
    // ==========================================
    gachaData: {
        isOpening: false,
        cost: 500
    ...
    openGacha() {
        const email = localStorage.getItem('haruno_email');
        if (!email) { this.openAuthModal(); return this.showToast("Cần đăng nhập để thử nhân phẩm!", "error"); }
        document.getElementById('gacha-modal').style.display = 'flex';
        this.resetGachaUI();
    },

    closeGacha() {
        if (this.gachaData.isOpening) return this.showToast("Đang bóc tem hộp, không được bỏ đi!", "warning");
    ...
    resetGachaUI() {
        ...
        document.getElementById('gacha-box').className = 'gacha-box-container';
        document.getElementById('gacha-box').innerHTML = '<i class="fas fa-box"></i>';
        
        document.getElementById('gacha-result').style.display = 'none';
        document.getElementById('gacha-rays').className = 'gacha-light-rays';
        document.getElementById('gacha-rays').style.background = 'radial-gradient(circle, rgba(255,255,255,0.2) 0%, transparent 60%)';

        document.getElementById('gacha-msg').innerText = '"Một phát lên mây, hay là ra đê?"';
        document.getElementById('gacha-msg').style.color = '#aaa';

        const btn = document.getElementById('btn-open-gacha');
        btn.disabled = false;
        btn.innerHTML = '<i class="fas fa-key"></i> MỞ 1 HỘP (500 HCOINS)';
    },
    ...
    openGachaBox() {
        ...
        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ĐANG TRẢ TIỀN...';

        // 1. Trừ tiền (500 HCoins)
        fetch("https://throbbing-disk-3bb3.thienbm101102.workers.dev", {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'deductMinigameFee', safeKey: safeUser, cost: this.gachaData.cost })
        }).then(res => res.json()).then(data => {
            if (!data.success) {
                this.showToast("Cháy túi rồi, nạp thêm đi Chủ Tịch!", "error");
                this.resetGachaUI();
            ...
            // [ÂM THANH] Trừ tiền & Bắt đầu rung lắc hộp
            this.playSound('coin');
            if(this.sounds && this.sounds.shuffle) {
                this.sounds.shuffle.loop = true;
                this.playSound('shuffle');
            }

            // Giao diện: Bắt đầu lắc hộp
            btn.innerHTML = '<i class="fas fa-box-open"></i> ĐANG MỞ HỘP...';
            document.getElementById('gacha-msg').innerText = 'Hộp đang rung lắc dữ dội...';
            document.getElementById('gacha-msg').style.color = '#ffcc00';
            
            const box = document.getElementById('gacha-box');
            box.classList.add('shaking');

            // 2. Chờ 2.5s để tạo sự hồi hộp, sau đó hiển thị kết quả
            setTimeout(() => {
                if(this.sounds && this.sounds.shuffle) {
                    this.sounds.shuffle.pause();
                    this.sounds.shuffle.currentTime = 0;
                }
                this.revealGachaResult(safeUser);
            }, 2500);

        }).catch(err => {
            this.showToast("Lỗi kết nối vũ trụ!", "error");
            this.resetGachaUI();
        });
    },

    revealGachaResult(safeUser) {
        // Thuật toán Tỷ lệ (Rates)
        const rand = Math.random() * 100;
        let rarity, itemName, icon, reward, colorClass, rayColor;

        if (rand < 5) {
            // 5% VÀNG (Legendary) - Trúng Độc Đắc
            rarity = 'legendary'; itemName = 'CÚP HOÀNG KIM'; icon = 'fa-trophy'; reward = 5000;
            colorClass = 'gacha-legendary'; rayColor = 'rgba(255, 215, 0, 0.4)';
            this.playSound('win');
            if (typeof this.fireJackpotEffect === "function") this.fireJackpotEffect();
        } else if (rand < 20) {
            // 15% TÍM (Epic) - Lời to
            rarity = 'epic'; itemName = 'RƯƠNG MA THUẬT'; icon = 'fa-gem'; reward = 1000;
            colorClass = 'gacha-epic'; rayColor = 'rgba(176, 80, 255, 0.4)';
            this.playSound('win');
        } else if (rand < 50) {
            // 30% XANH (Rare) - Lỗ một xíu
            rarity = 'rare'; itemName = 'TÚI VÀNG NHỎ'; icon = 'fa-sack-dollar'; reward = 300;
            colorClass = 'gacha-rare'; rayColor = 'rgba(77, 166, 255, 0.4)';
            this.playSound('click'); // Nhạc bình thường
        } else {
            // 50% TRẮNG (Common) - Lỗ sặc máu
            rarity = 'common'; itemName = 'CỤC ĐÁ VÔ DỤNG'; icon = 'fa-cubes'; reward = 100;
            colorClass = 'gacha-common'; rayColor = 'rgba(255, 255, 255, 0.2)';
            this.playSound('fail'); // Nhạc tạch
        }

        // Cập nhật Giao diện
        document.getElementById('gacha-box').style.display = 'none'; // Ẩn hộp
        
        const resultDiv = document.getElementById('gacha-result');
        resultDiv.style.display = 'flex';
        
        document.getElementById('gacha-item-icon').innerHTML = `<i class="fas ${icon} ${colorClass}"></i>`;
        document.getElementById('gacha-item-name').className = `gacha-item-name ${colorClass}`;
        document.getElementById('gacha-item-name').innerText = itemName;
        
        document.getElementById('gacha-item-value').className = `gacha-item-value ${colorClass}`;
        document.getElementById('gacha-item-value').innerText = `Nhận lại: +${reward.toLocaleString()} HCoins`;

        // Bật tia sáng phía sau
        const rays = document.getElementById('gacha-rays');
        rays.classList.add('active');
        rays.style.background = `radial-gradient(circle, ${rayColor} 0%, transparent 60%)`;

        // Cập nhật lời nhắn
        let msg = "";
        if(reward > this.gachaData.cost) msg = `Tuyệt vời! Bạn đã lời to! (Lãi ${reward - this.gachaData.cost} HCoins)`;
        else msg = `Đen thôi đỏ quên đi! (Lỗ ${this.gachaData.cost - reward} HCoins)`;
        
        document.getElementById('gacha-msg').innerText = msg;
        document.getElementById('gacha-msg').style.color = '#fff';

        // Gọi API cộng tiền thưởng về túi
        fetch("https://throbbing-disk-3bb3.thienbm101102.workers.dev", {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'minigameResult', safeKey: safeUser, amount: reward })
        });

        // Cho phép mở tiếp sau 2 giây
        setTimeout(() => {
            const btn = document.getElementById('btn-open-gacha');
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-redo"></i> MỞ THÊM HỘP NỮA (500 HCOINS)';
            this.gachaData.isOpening = false;
        }, 2000);
    },
};
