...
// GỌI HÀM QUÉT KHO ĐỒ VÀ MỞ KHÓA VẬT PHẨM
    ...
    createMovieCard(m, isHorizontal = false) {
        ...
        // Tách biệt rõ ràng Chất lượng và Ngôn ngữ
        const quality = m.quality || 'HD';
        const lang = m.language || m.lang || 'Vietsub';
        
        // Trạng thái tập phim vẫn giữ để hiện trên huy hiệu (Badge) của ảnh
                ...
                <div class="hover-info">
                    <div class="hover-content">
                        ...
                        <div class="h-details-grid">
                            <div class="h-item"><span>Chất lượng</span> ${quality}</div>
                            <div class="h-item" title="${lang}"><span>Tình trạng</span> ${lang}</div>
    ...
    async renderMovies() {
        ...
        let urls = [];
        // Khai báo các chuyên mục cần xử lý filter ngầm
        const customTypes = ['hoat-hinh-trung-quoc', 'hoat-hinh-nhat-ban', 'hoat-hinh-khac', 'anime-custom'];
        ...
        for (let i = 0; i < pagesPerLoad; i++) {
            ...
            } else if (!customTypes.includes(this.currentType)) {
        ...
        try {
            ...
            if (customTypes.includes(this.currentType)) {
                // CƠ CHẾ QUÉT SÂU (DEEP SCAN) CHUYÊN DỤNG CHO HOẠT HÌNH
                ...
                while(allItems.length < targetCount && pagesScanned < maxPagesToScan) {
                    // LUÔN LẤY DANH SÁCH HOẠT HÌNH LÀM GỐC CHUẨN
                    ...
                    // GỌI API CHI TIẾT CỦA TỪNG PHIM ĐỂ PHÁ VỠ BỨC TƯỜNG GIẤU DATA
                    let detailPromises = tempItems.map(m => this.fetchWithCache(`${API_URL}/film/${m.slug}`, 3600));
                    let details = await Promise.all(detailPromises); // Chạy song song siêu tốc
                    
                    for (let i = 0; i < details.length; i++) {
                        let d = details[i];
                        if (!d) continue;
                        // Trích xuất data xịn từ bên trong trang chi tiết
                        let mDetail = d.movie || d.item || d.data?.item || d.data?.movie;
                        if (!mDetail) continue;

                        // Bây giờ Quốc gia đã lộ diện, ta dùng getTaxonomy để phân loại
                        const countries = this.getTaxonomy(mDetail, 'country');
                        const isTrung = countries.some(c => c.toLowerCase().includes('trung quốc') || c.toLowerCase().includes('china'));
                        const isNhat = countries.some(c => c.toLowerCase().includes('nhật bản') || c.toLowerCase().includes('japan'));
                        
                        // Phân bổ đúng vào mục bạn chọn
                        if (this.currentType === 'hoat-hinh-trung-quoc' && isTrung) {
                            allItems.push(mDetail);
                        } else if ((this.currentType === 'hoat-hinh-nhat-ban' || this.currentType === 'anime-custom') && isNhat) {
                            allItems.push(mDetail);
                        } else if (this.currentType === 'hoat-hinh-khac' && !isTrung && !isNhat) {
                            allItems.push(mDetail);
                        }
                ...
                totalPages = this.currentPage + 1; // Tạo ảo giác vô tận cho phân trang
            } else {
                // XỬ LÝ CHO CÁC DANH MỤC BÌNH THƯỜNG KHÁC
            ...
            grid.innerHTML = ''; // Xóa lớp sương mù skeleton
            if (uniqueItems.length > 0) {
                // Dùng Fragment chống giật lag
    ...
    openDashboard() {
        ...
        if (rankEl) {
            // Lấy email và tạo safeKey
            const email = localStorage.getItem('haruno_email');
            const safeUser = app.getSafeKey(email);
            
            // GỌI TRỰC TIẾP HÀM SINH HUY HIỆU CỦA BẠN VÀO ĐÂY
            rankEl.innerHTML = app.getFinalBadge(safeUser, isPremium);
            
            // Làm trong suốt cái khung chứa bên ngoài để không bị đè CSS lên cái Badge
            rankEl.style.background = 'transparent';
            rankEl.style.border = 'none';
            rankEl.style.boxShadow = 'none';
            rankEl.style.padding = '0';
        }
    ...
    renderDashboardData() {
        ...
        const itemDictionary = {
            ...
            'effect-spiderman': { name: 'Spiderman', image: 'https://i.ibb.co/8D0qD6nZ/ezgif-7019d12b2bcd97e7.gif', color: '#e67e22', rarity: 'Cửa Hàng' },
            'effect-venom': { name: 'Venom', image: 'https://i.ibb.co/SDv0cxyN/ezgif-129f620f5f8c1991.gif', color: '#e67e22', rarity: 'Cửa Hàng' },
            'effect-gomah': { name: 'Gomah', image: 'https://i.ibb.co/5WdXZ5LV/ezgif-105546704f05d7b6.gif', color: '#e67e22', rarity: 'Cửa Hàng' },
            'effect-goku': { name: 'Goku Mini', image: 'https://i.ibb.co/XxPg8Wk0/ezgif-1e9300f69c4679a5.gif', color: '#e67e22', rarity: 'Cửa Hàng' },
            'effect-vegeta': { name: 'Vegeta Mini', image: 'https://i.ibb.co/Kjs8rxJm/ezgif-1e5c921f61ebbc97.gif', color: '#e67e22', rarity: 'Cửa Hàng' },
            'effect-piccolo': { name: 'Piccolo Mini', image: 'https://i.ibb.co/s9sGcdtj/ezgif-1f6af429c6eab985.gif', color: '#e67e22', rarity: 'Cửa Hàng' },
			'effect-1': { name: 'Hello Kitty', image: 'https://i.ibb.co/cK68ddk5/ezgif-5e5255d01cb7216f.gif', color: '#e67e22', rarity: "Giới Hạn" },
            'effect-2': { name: 'Pompompurin', image: 'https://i.ibb.co/PsnXR2wy/ezgif-5ede70dcde25a82f.gif', color: '#e67e22', rarity: "Giới Hạn" },
            'effect-3': { name: 'Cinnamoroll', image: 'https://i.ibb.co/ybftR2X/ezgif-5506e24774635a62.gif', color: '#e67e22', rarity: "Giới Hạn" },
            'effect-4': { name: 'Kuromi x Melody', image: 'https://i.ibb.co/F46tFfqr/ezgif-574f6bdffdaf259d.gif', color: '#e67e22', rarity: "Giới Hạn" },
            'effect-5': { name: 'LittleTwinStars', image: 'https://i.ibb.co/XfVNDk7H/ezgif-5d1cbcb7fb94860c.gif', color: '#e67e22', rarity: "Giới Hạn" },
            'effect-6': { name: 'Chơi Trong Sân', image: 'https://i.ibb.co/SWgcXSv/ezgif-5feebf3c8a69109c.gif', color: '#e67e22', rarity: "Giới Hạn" },
            
            // --- CÁC KHUNG AVATAR ---
            'frame-yunara': { name: 'Yunara', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_6f59e75226ea65207068cf672c35b023', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-shoto': { name: 'Shoto', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_9e815a5c371894d0ce5a15fba9cf999a', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-pandora': { name: 'Pandora', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_d2bf761ee4331af2b64ff9294ecf229f', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-shenron': { name: 'Shenron', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_3ce0ab2726378ba762fcc5318b87ee6f', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-spiderman': { name: 'Spiderman', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1481384635886862397/animated', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-venom': { name: 'Venom', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1481387347642810480/animated', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-ngocrong': { name: 'Ngọc Rồng', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_f1a2dc0cceccbc3da0838627460bd6ee', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-gomah': { name: 'Gomah', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_4cb3f6e0d7869b30b2ed561dd205e7f8', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-goku': { name: 'Goku Mini', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_32b48fb7742f69d1f870bccfeb42044d', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-vegeta': { name: 'Vegeta Mini', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_25e6d6c44079a0c58648295e591968b0', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-glorio': { name: 'Glorio', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_2bd0c5be907e9e41cb3bfcf54a60e03e', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-kai': { name: 'Supreme Kai Mini', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_89f8a8380086df9c310ce007fad36f33', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-piccolo': { name: 'Piccolo Mini', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_fe93404cffc76809692f753f4311234b', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-panzy': { name: 'Panzy', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_918f5d4088854a5d12cfb0e9b821396d', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-bantim': { name: 'Bắn Tim', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_aa8dcf830fe7026c422139055c58546c', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-doichan': { name: 'Đôi Chân Vỗ Nhịp', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_dce2396544ebe23aa6a5194c7d6cde94', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-um': { name: 'Ừm, Thực Ra Thì...', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_5cbb9718e2dfa11903327526d23eb0fd', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-banphim': { name: 'Anh Hùng Bàn Phím', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_03df56ee300b381fdb1368609f1d921d', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-slay': { name: 'Slay', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_853f18504364be844e95b4ad85e2a0f6', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-andam': { name: 'Muốn Ăn Đấm Không?', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_490c2310195e1403c68b73301f083929', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-ngamsao': { name: 'Ngắm Sao Lấp Lánh', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_f37320babca6e37d8392950cd1a9fc4c', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-thucan': { name: 'Thức Ăn Cho Tâm Trí', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_7864a49617b3524cf473adfd508aa651', color: '#e67e22', rarity: 'Cửa Hàng' },
            'frame-nani': { name: 'Khung Nani!?', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_d3f20f04744b398451686b5229505fea', color: '#e67e22', rarity: 'Cửa Hàng' },
			'frame-1': { name: 'Hello Kitty', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_1ab42e495777eb9e8728a6c636b0a954', color: '#e67e22', rarity: 'Giới Hạn' },
			'frame-2': { name: 'Pompompurin', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_a16394f64b8d9fa38fa75078dc408689', color: '#e67e22', rarity: 'Giới Hạn' },
			'frame-3': { name: 'Cinnamoroll', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_4786361f20944a2dfe2c55986ee79571', color: '#e67e22', rarity: 'Giới Hạn' },
			'frame-4': { name: 'Pochacco', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_8dfc22a5f29064737dd2628b153da17a', color: '#e67e22', rarity: 'Giới Hạn' },
			'frame-5': { name: 'Kuromi', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_d6b900c052726061be62b8ff4278d135', color: '#e67e22', rarity: 'Giới Hạn' },
			'frame-6': { name: 'My Melody', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_35784f5c1ae9662eecddba177c0a21a3', color: '#e67e22', rarity: 'Giới Hạn' },
			'frame-7': { name: 'LittleTwinStars', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_bb6cedd7a96db71ac1e3eb5392f03a0b', color: '#e67e22', rarity: 'Giới Hạn' },
			'frame-8': { name: 'Chococat', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_ba85f37e87d48950a6cc7135c6302afb', color: '#e67e22', rarity: 'Giới Hạn' },
			'frame-9': { name: 'Keroppi', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_451ba8354eb5749a606c8a3a89970064', color: '#e67e22', rarity: 'Giới Hạn' },
			'frame-10': { name: 'Tuxedosam', image: 'https://cdn.discordapp.com/avatar-decoration-presets/a_ee77df4c2dc6f8aa9dc11aacc1effcd8', color: '#e67e22', rarity: 'Giới Hạn' },
            
            // --- CÁC KHUNG CHAT (Ảnh thật từ Server của bạn) ---
            'chat-effect-1': { name: 'Người Nhện', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1481388758455550114/animated', color: '#e67e22', rarity: 'Cửa Hàng' },
            'chat-effect-2': { name: 'Venom', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1481389947515830282/animated', color: '#e67e22', rarity: 'Cửa Hàng' },
            'chat-effect-3': { name: 'Người Nhện vs. Venom', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1481390594810183700/animated', color: '#e67e22', rarity: 'Cửa Hàng' },
            'chat-effect-4': { name: 'Gomah', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1400163655399641249/animated', color: '#e67e22', rarity: 'Cửa Hàng' },
            'chat-effect-5': { name: 'Vegeta Mini', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1400163655424933978/animated', color: '#e67e22', rarity: 'Cửa Hàng' },
            'chat-effect-6': { name: 'Goku Mini', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1400163655462555658/animated', color: '#e67e22', rarity: 'Cửa Hàng' },
            'chat-effect-7': { name: 'Ngọc Rồng', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1400163655487848501/animated', color: '#e67e22', rarity: 'Cửa Hàng' },
			'chat-effect-8': { name: 'Hello Kitty', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1488244924066566185/animated', color: '#e67e22', rarity: 'Giới Hạn' },
			'chat-effect-9': { name: 'Pompompurin', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1488245189482123364/animated', color: '#e67e22', rarity: 'Giới Hạn' },
			'chat-effect-10': { name: 'Cinnamoroll', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1488245478213816320/animated', color: '#e67e22', rarity: 'Giới Hạn' },
			'chat-effect-11': { name: 'Pochacco', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1488245662947475506/animated', color: '#e67e22', rarity: 'Giới Hạn' },
			'chat-effect-12': { name: 'Kuromi x Melody', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1488245817364975626/animated', color: '#e67e22', rarity: 'Giới Hạn' },
			'chat-effect-13': { name: 'Chococat', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1488245996054904983/animated', color: '#e67e22', rarity: 'Giới Hạn' },
			'chat-effect-14': { name: 'gudetama', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1488246136585060452/animated', color: '#e67e22', rarity: 'Giới Hạn' },
			'chat-effect-15': { name: 'Badtz-maru', image: 'https://cdn.discordapp.com/media/v1/collectibles-shop/1488246309197713542/animated', color: '#e67e22', rarity: 'Giới Hạn' }
        };
    ...
    async joinVoiceRoom(roomId, roomName) {
        ...
        // Lấy Tên và Avatar chính xác từ dữ liệu hệ thống (Hỗ trợ VIP)
        ...
        // Khởi tạo Micro với cơ chế chống lỗi phần cứng mượt mà
        try {
            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                throw new Error("Trình duyệt không hỗ trợ. Vui lòng dùng HTTPS hoặc localhost.");
            }
            ...
            this.showToast("Micro đang bị kẹt hoặc tab khác chiếm dụng. Bạn tạm thời chỉ có thể nghe!", "warning");
        ...
        this.voicePeer.on('call', call => {
            if (this.localStream) {
                ...
                call.answer(); // Trả lời không có luồng (chỉ nhận âm thanh từ đối phương)
            }
    ...
    toggleDeafen() {
        ...
        if (this.isDeafened) {
            btn.classList.add('active');
            btn.innerHTML = `<i class="fas fa-deaf"></i> Tắt Âm (Bật)`;
            if (!this.isMuted) this.toggleMic(); // Bật điếc thì tự động mute mic
        } else {
            btn.classList.remove('active');
            btn.innerHTML = `<i class="fas fa-headphones"></i> Tắt Âm`;
        }
    },
    ...
    listenVoiceRoomChat() {
        ...
        db.ref(`voice_rooms/${this.voiceRoomId}/messages`).limitToLast(30).on('value', snap => {
            ...
            snap.forEach(child => {
                ...
                // Tích hợp hiệu ứng Premium vào khung chat Voice
                    ...
                    resultHtml = topMovies.map(movie => {
                        ...
                        <div class="search-item" onclick="app.showMovie('${movie.slug}')">
                            ...
                            <div class="search-item-info">
                                ...
                                <div class="sub-title">${originName} </div>
                            </div>
                        </div>
                    `}).join('');
                } else {
