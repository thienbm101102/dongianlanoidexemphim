...
// ==========================================
// HỆ THỐNG TIẾN LÊN MIỀN NAM: CHUẨN ZINGPLAY (BẢN FINAL - ĐÃ FIX LỖI BẮT ĐẦU)
// ==========================================
app.tlRoomId = null;
app.tlTimer = null; 
...
app.tl_startGameOnline = function() {
    ...
    db.ref(`tlmn_rooms/${this.tlRoomId}`).once('value').then(async snap => {
        ...
        // CHECK ĐỦ NGƯỜI VÀ BÁO LỖI RÕ RÀNG
        if (playerKeys.length < 2) { 
            alert("⚠️ Phải có ít nhất 2 người trong phòng mới có thể chia bài!\n\nHãy mở thêm 1 tab ẩn danh, đăng nhập nick phụ vào phòng này để test nhé.");
            return; 
        }

        // KHÔNG TRỪ TIỀN Ở ĐÂY NỮA (VÌ ĐÃ TRỪ LÚC TẠO/VÀO PHÒNG RỒI)
        let validPlayers = {};
        for(let pk of playerKeys) {
            validPlayers[pk] = room.players[pk]; 
        }
        ...
        // BẮT ĐẦU VÁN - GHI LÊN FIREBASE VỚI GIỜ CHUẨN
        db.ref(`tlmn_rooms/${this.tlRoomId}`).update({
