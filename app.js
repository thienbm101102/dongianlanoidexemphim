...
// ==========================================
	...
	// ==========================================
    // MINIGAME MÊ CUNG TỬ THẦN (DUNGEON RPG)
    // ==========================================
    dungeonData: {
        ...
        maxHp: 3,
        multiplier: 1.00,
        grid: [], // Lưu trữ 25 ô: 'gold', 'monster', 'potion'
        tilesOpened: 0,
        goldCount: 15, // Số lượng Vàng
        monsterCount: 7, // Số lượng Quái
        potionCount: 3 // Số lượng Máu
    },

    openDungeonGame() {
        const email = localStorage.getItem('haruno_email');
        if (!email) { this.openAuthModal(); return this.showToast("Cần đăng nhập để vào Hầm Ngục!", "error"); }
        document.getElementById('dungeon-modal').style.display = 'flex';
        this.resetDungeonUI();
    },

    closeDungeonGame() {
        if (this.dungeonData.state === 'playing') return this.showToast("Đang kẹt trong hầm ngục, hãy tìm đường thoát!", "warning");
        document.getElementById('dungeon-modal').style.display = 'none';
    },

    resetDungeonUI() {
        this.dungeonData.state = 'idle';
        this.dungeonData.hp = 3;
        this.dungeonData.multiplier = 1.00;
        this.dungeonData.tilesOpened = 0;
        
        document.getElementById('dungeon-bet-amount').disabled = false;
        document.getElementById('dungeon-profit-box').style.display = 'none';
        document.getElementById('dungeon-action-area').style.display = 'block';
        document.getElementById('dungeon-gameplay-area').style.display = 'none';
        document.getElementById('dungeon-modal').querySelector('.glass-tour-board').classList.remove('dg-damage-anim');

        this.updateDungeonHP();
        this.updateDungeonMsg("CHỌN MỨC CƯỢC ĐỂ MỞ CỬA", "#fff");
        this.renderDungeonGrid(true); // Tham số true để vẽ bảng trống
    },

    updateDungeonHP() {
        const hpContainer = document.getElementById('dungeon-hp-display');
        hpContainer.innerHTML = '';
        for(let i=0; i < this.dungeonData.maxHp; i++) {
            if(i < this.dungeonData.hp) hpContainer.innerHTML += '<i class="fas fa-heart active"></i> ';
            else hpContainer.innerHTML += '<i class="fas fa-heart"></i> ';
        }
    },

    updateDungeonMsg(text, color) {
        const msgEl = document.getElementById('dungeon-status-msg');
        msgEl.innerText = text;
        msgEl.style.color = color;
    },

    startDungeonGame() {
        const betInput = document.getElementById('dungeon-bet-amount').value;
        const betAmount = parseInt(betInput);
        if (isNaN(betAmount) || betAmount <= 0) return this.showToast("Cược không hợp lệ!", "error");

        const email = localStorage.getItem('haruno_email');
        const safeUser = this.getSafeKey(email);
        const btn = document.getElementById('btn-dungeon-start');

        btn.disabled = true;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ĐANG MỞ CỬA...';
        ...
        fetch("https://throbbing-disk-3bb3.thienbm101102.workers.dev", {
            ...
            // Khởi tạo Game
            this.dungeonData.bet = betAmount;
            this.dungeonData.state = 'playing';
            this.dungeonData.hp = 3;
            this.dungeonData.multiplier = 1.00;
            this.dungeonData.tilesOpened = 0;

            // Thuật toán nhồi đồ vào hầm ngục và trộn lên (Shuffle)
            let items = [];
            for(let i=0; i<this.dungeonData.goldCount; i++) items.push('gold');
            for(let i=0; i<this.dungeonData.monsterCount; i++) items.push('monster');
            for(let i=0; i<this.dungeonData.potionCount; i++) items.push('potion');
            ...
            this.dungeonData.grid = items;

            // UI
            document.getElementById('dungeon-bet-amount').disabled = true;
            document.getElementById('dungeon-profit-box').style.display = 'block';
            document.getElementById('dungeon-current-profit').innerText = betAmount.toLocaleString();
            document.getElementById('dg-multi').innerText = "1.00";
            
            document.getElementById('dungeon-action-area').style.display = 'none';
            document.getElementById('dungeon-gameplay-area').style.display = 'block';
            
            this.updateDungeonHP();
            this.updateDungeonMsg("LẬT GẠCH ĐỂ TÌM KHO BÁU!", "#00ffcc");
            this.renderDungeonGrid(); // Vẽ bảng chơi
        });
    },

    renderDungeonGrid(isEmpty = false) {
        const grid = document.getElementById('dungeon-grid');
        grid.innerHTML = '';
        for (let i = 0; i < 25; i++) {
            const tileHtml = `<div class="dg-tile" id="dg-tile-${i}" onclick="app.clickDungeonTile(${i})">
                <i class="fas fa-question" style="color: #444; font-size: 20px;"></i>
                <div class="tile-inner" id="dg-inner-${i}"></div>
            </div>`;
            grid.innerHTML += tileHtml;
        }
    },

    clickDungeonTile(index) {
        if (this.dungeonData.state !== 'playing') return;
        
        const tile = document.getElementById(`dg-tile-${index}`);
        if (tile.classList.contains('revealed')) return; // Không cho click lại
        
        const type = this.dungeonData.grid[index];
        const inner = document.getElementById(`dg-inner-${index}`);
        
        tile.classList.add('revealed');
        this.dungeonData.tilesOpened++;

        if (type === 'gold') {
            // ĂN VÀNG: Tăng hệ số x1.15
            inner.innerHTML = '<i class="fas fa-coins"></i>';
            tile.classList.add('dg-gold');
            if(this.sounds) this.playSound('coin');
            
            this.dungeonData.multiplier *= 1.15; // Lãi kép
            this.updateDungeonProfit();
            this.updateDungeonMsg("TÌM THẤY VÀNG! NHÂN HỆ SỐ!", "#ffd700");

            // Hiệu ứng text bay lên
            this.createFlyText(tile, "+15%");

        } else if (type === 'potion') {
            // ĂN MÁU
            inner.innerHTML = '<i class="fas fa-flask"></i>';
            tile.classList.add('dg-potion');
            if(this.sounds) this.playSound('win'); // Tiếng ting

            if (this.dungeonData.hp < this.dungeonData.maxHp) {
                this.dungeonData.hp++;
                this.updateDungeonHP();
                this.updateDungeonMsg("HỒI 1 HP! TIẾP TỤC CHIẾN ĐẤU!", "#00ffcc");
                this.createFlyText(tile, "+1 HP");
            } else {
                this.updateDungeonMsg("MÁU ĐÃ ĐẦY! BÌNH THUỐC BỊ BỎ PHÍ!", "#aaa");
            }

        } else if (type === 'monster') {
            // GẶP QUÁI
            inner.innerHTML = '<i class="fas fa-dragon"></i>';
            tile.classList.add('dg-monster');
            if(this.sounds) this.playSound('fail');

            this.dungeonData.hp--;
            this.updateDungeonHP();
            this.createFlyText(tile, "-1 HP", "#ff3333");

            // Hiệu ứng rung màn hình
            const board = document.getElementById('dungeon-modal').querySelector('.glass-tour-board');
            board.classList.remove('dg-damage-anim');
            void board.offsetWidth; // trigger reflow
            board.classList.add('dg-damage-anim');

            if (this.dungeonData.hp <= 0) {
                // CHẾT - GAME OVER
                this.updateDungeonMsg("BẠN ĐÃ TỬ TRẬN! MẤT TOÀN BỘ KHO BÁU!", "#ff3333");
                this.endDungeon(false);
                return;
            } else {
                this.updateDungeonMsg("BỊ QUÁI VẬT CẮN! TRỪ 1 HP!", "#ff3333");
            }
        }

        // Tự động Cashout nếu đã mở hết số Vàng (15 cục vàng)
        // Đây là trường hợp phá đảo hầm ngục
        let openedGolds = 0;
        for(let i=0; i<25; i++) {
            const t = document.getElementById(`dg-tile-${i}`);
            if(t.classList.contains('revealed') && this.dungeonData.grid[i] === 'gold') openedGolds++;
        }
        
        if (openedGolds === this.dungeonData.goldCount) {
            this.updateDungeonMsg("PHÁ ĐẢO HẦM NGỤC! ÔM TRỌN KHO BÁU!", "#ffd700");
            if(typeof this.fireJackpotEffect === 'function') this.fireJackpotEffect();
            this.cashOutDungeon();
        }
    },

    createFlyText(tileEl, text, color = "#ffd700") {
        const flyEl = document.createElement('div');
        flyEl.className = 'fly-text';
        flyEl.innerText = text;
        flyEl.style.color = color;
        tileEl.appendChild(flyEl);
        setTimeout(() => flyEl.remove(), 1000);
    },

    updateDungeonProfit() {
        const profit = Math.floor(this.dungeonData.bet * this.dungeonData.multiplier);
        document.getElementById('dungeon-current-profit').innerText = profit.toLocaleString();
        document.getElementById('dg-multi').innerText = this.dungeonData.multiplier.toFixed(2);
    },

    cashOutDungeon() {
        if (this.dungeonData.state !== 'playing') return;
        this.updateDungeonMsg("ĐANG TẨU THOÁT...", "#00ffcc");
        this.endDungeon(true);
    },

    endDungeon(isWin) {
        this.dungeonData.state = 'ended';
        document.getElementById('btn-dungeon-cashout').disabled = true;

        // Lật toàn bộ các ô còn lại để người chơi xem (làm mờ đi)
        for (let i = 0; i < 25; i++) {
            const tile = document.getElementById(`dg-tile-${i}`);
            if (!tile.classList.contains('revealed')) {
                const type = this.dungeonData.grid[i];
                const inner = document.getElementById(`dg-inner-${i}`);
                
                if (type === 'gold') inner.innerHTML = '<i class="fas fa-coins"></i>';
                else if (type === 'potion') inner.innerHTML = '<i class="fas fa-flask"></i>';
                else inner.innerHTML = '<i class="fas fa-dragon"></i>';
                
                tile.classList.add('revealed', `dg-${type}`, 'dg-dimmed');
            }
        }
        ...
        if (isWin) {
            ...
            const profit = Math.floor(this.dungeonData.bet * this.dungeonData.multiplier);
            this.showToast(`Thoát hiểm thành công! Nhận ${profit.toLocaleString()} HCoins.`, "success");

            const email = localStorage.getItem('haruno_email');
            const safeUser = this.getSafeKey(email);
            fetch("https://throbbing-disk-3bb3.thienbm101102.workers.dev", {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ action: 'minigameResult', safeKey: safeUser, amount: profit })
            });
        } else {
            this.showToast("Bạn đã bỏ mạng tại Mê Cung Tử Thần.", "error");
        }
